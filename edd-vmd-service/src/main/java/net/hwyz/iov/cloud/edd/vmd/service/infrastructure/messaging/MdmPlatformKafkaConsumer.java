 ...
 * {@code vmd.kafka.topics.mdm.platform} 提供，归 EDD-MDM 管理，VMD 只消费不创建），
 * 转换为本地MdmPlatformEvent并调用 MdmSyncAppService.handlePlatformEvent() 进行幂等upsert。
 * </p>
 *
 * @author CR-024
...
public class MdmPlatformKafkaConsumer {
     ...
     */
    @KafkaListener(
            id = MdmProjectionType.ConsumerIds.PLATFORM,
            topics = {"${vmd.kafka.topics.mdm.platform:mdm.platform}"},
    ...
    public void onPlatformEvent(ConsumerRecord<String, String> record) {
        ...
        MdmPlatformEvent event;
        try {
            event = parseEvent(record.value());
        } catch (Exception e) {
            mdmSyncMetrics.recordFailure();
            mdmConsumerMetrics.recordConsume(MdmProjectionType.PLATFORM, "parse_error");
            log.error("MDM Platform事件解析失败: offset={}, error={}",
        ...
        try {
            ...
            mdmConsumerMetrics.recordConsume(MdmProjectionType.PLATFORM, "success");
            log.info("MDM Platform事件处理成功: entityId={}, eventType={}",
                    event.getEntityId(), event.getEventType());
        } catch (Exception e) {
            mdmSyncMetrics.recordFailure();
            mdmConsumerMetrics.recordConsume(MdmProjectionType.PLATFORM, "failure");
