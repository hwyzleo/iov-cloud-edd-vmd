...
/**
 * MdmModelRepository集成测试
 *
 * @author hwyz_leo
 */
@Rollback
class MdmModelRepositoryTest extends BaseTest {
    ...
    void insert_shouldSuccessfullyInsertModel() {
        ...
        // When
        int result = mdmModelRepository.insert(model);
    ...
    void selectById_shouldReturnModelWhenExists() {
        ...
        // When
        Model result = mdmModelRepository.selectById(model.getId());
    ...
    void selectByCode_shouldReturnModelWhenCodeExists() {
        ...
        // When
        Model result = mdmModelRepository.selectByCode(code);
    ...
    void batchPhysicalDelete_shouldSuccessfullyDeleteModels() {
        ...
        mdmModelRepository.insert(model2);

        Long[] ids = {model1.getId(), model2.getId()};

        // When
        int result = mdmModelRepository.batchPhysicalDelete(ids);

        // Then
        assertEquals(2, result);
        assertNull(mdmModelRepository.selectById(model1.getId()));
        assertNull(mdmModelRepository.selectById(model2.getId()));
    }
    ...
    void selectByMap_shouldReturnModelsMatchingCriteria() {
        ...
        // When
        List<Model> result = mdmModelRepository.selectByMap(map);
    ...
    void countByMap_shouldReturnCorrectCount() {
        ...
        // When
        int result = mdmModelRepository.countByMap(map);
    ...
    void countBySource_shouldReturnCountForSource() {
        ...
        mdmModelRepository.insert(modelManual);

        // When
        long mdmCount = mdmModelRepository.countBySource(SourceType.MDM);
        long manualCount = mdmModelRepository.countBySource(SourceType.MANUAL);
    ...
    void insert_shouldSuccessfullyInsertMdmModelWithProjectionFields() {
        ...
        Model insertedModel = mdmModelRepository.selectById(model.getId());
