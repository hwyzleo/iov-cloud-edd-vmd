...
| D13 | MDM 同步策略 | Kafka 事件订阅为主 + Feign 全量快照兜底 + 本地 source 字段标注来源 | ① 仅 Feign 轮询 ② 仅 Kafka 事件 ③ 数据库 CDC | **解耦**：Kafka 事件异步消费，MDM 不可用时不阻塞 VMD 主流程。**可重放**：Kafka 事件支持 offset 回溯重放。**支持降级**：MDM 不可达时降级为只读，source=MANUAL 记录仍可本地维护。**幂等**：通过 external_ref_id + external_version 保证 upsert 幂等性。**适用实体（CR-011/CR-015/CR-016/CR-017 扩展）**：品牌 / 车系 / 平台 / **Plant（工厂）**/ **车型（Model）**/ **版本（Variant，原 BaseModel 基础车型）**/ **配置（Configuration，原 BuildConfig 生产配置）**统一采用本策略。详见「edd-mdm 接入规范」 |
...
| `veh_build_config_feature_code` | `VehBuildConfigFeatureCodePo` | `configuration_code`（原 `build_config_code`）, `family_code`, `feature_code` | UK(`configuration_code`,`family_code`) | → `veh_configuration.code`；**CR-017 仅引用键 `build_config_code`→`configuration_code` 兼容改名，表名与 BuildConfigFeatureCode 实体名不变、特征值业务语义不变**（旧列兼容期保留），继续支撑 US-031 反查；最终归属与 FeatureFamily 改造留待 CR-018 |
...
**对应 US**：**US-001** / US-002 / US-003 / **US-004** / **US-005** / US-006 / **US-007**（MDM 事件同步 AC）。
...
sequenceDiagram
    ...
    participant FC as MdmBrandQueryClient / MdmCarLineQueryClient / MdmPlatformQueryClient / MdmPlantQueryClient / MdmModelQueryClient / MdmVariantQueryClient / MdmConfigurationQueryClient
    ...
    alt 启动时自动
        ...
        VMD->>C: POST /api/mpt/mdmSync/v1/bootstrap?entity=brand|carLine|platform|plant|model|variant|configuration|all
...
**对应 US**：**US-001b** / US-002b / US-003b / **US-004b** / **US-005b** / US-006b / **US-007b**（Bootstrap 全量同步 AC）。
...
#### 5.1.17 MdmSync `MptMdmSyncController`（→ US-001b/US-002b/US-003b/US-004b/US-005b/US-006b/US-007b）
| Method | Path | Permission | Request | Response |
|--------|------|-----------|---------|----------|
| POST | `/api/mpt/mdmSync/v1/bootstrap` | `completeVehicle:mdmSync:bootstrap` | `?entity=brand\|carLine\|platform\|plant\|model\|variant\|configuration\|all` | `ApiResponse<String>` |

**行为**：调用 MDM 全量快照接口拉取指定实体数据并 upsert 本地副本（不删除本地记录）。`entity=plant` 同步 Plant 投影（CR-011），`entity=model` 同步 Model 投影（CR-015），`entity=variant` 同步 Variant 投影（CR-016），`entity=configuration` 同步 Configuration 投影（CR-017），`entity=all` 含 Plant、Model、Variant 与 Configuration。
...
#### 5.2.1 MDM 快照查询 Feign 接口（→ US-001b/US-002b/US-003b/US-004b/US-005b/US-006b/US-007b）
...
- `String getConfigurationCodeByFeatureCodes(@RequestParam Map<String,String> featureCodes)`（原 `getBuildConfigCodeByFeatureCodes`，对应路径 `/configurationCode`；CR-017 命名迁移，迁移期保留旧签名/旧路径 `/buildConfigCode` 兼容）
...
- `VmdConfigurationResponse getConfiguration(@PathVariable String configurationCode)`（原 `getBuildConfig`，对应路径 `/configuration/{configurationCode}`；CR-017 命名迁移，迁移期保留旧签名/旧路径 `/buildConfig/{buildConfigCode}` 与旧响应类型 `VmdBuildConfigResponse` 兼容）
...
| US-005 Configuration 投影（原 BuildConfig） | §2 D13/D20 / §3.1 产品树(`veh_configuration`)+物理车(`configuration_code`) / §3.2 / §3.4 V9/V10 / §4.6 F6 / §4.7 F7 / §5.1.5 / §5.2.1 / §5.2.5 | 消费 MDM Configuration 主数据本地投影（BuildConfig→Configuration 重命名迁移）；source=MDM 只读；按需最小化投影；保留特征值映射支撑 US-031 反查；保留「版本（Variant）→配置（Configuration）」引用链与每台物理车 `configurationCode` 唯一映射；BuildConfigFeatureCode 仅引用键兼容改名，FeatureFamily 留 CR-018（CR-017） |
...
| US-031 反查 Configuration（配置） | §5.2.5 / §3.1 产品树(`veh_configuration`)+`veh_build_config_feature_code` | 任意特征值组合反查 `configurationCode`；CR-017 路径/响应类型改 Configuration（`VmdConfigurationResponse`，旧路径/旧类型兼容），反查逻辑仍属 VMD（本地投影 + 特征值映射，不强依赖 MDM 运行时） |
