...
| `part_info` | `PartInfoPo` | `part_code`（=`pn`，→`part.pn` 引用键，NOT NULL）, `sn`, `vehicle_node_code`（节点归属，→`mdm_vehicle_node.code`）, `hardware_ver`, `software_ver`, `hardware_pn`, `software_pn`, `config_word`, `batch_num`, `supplier_code`（透传）, `extra`, `instance_state`, `first_seen_time`, `source`(N/A，VMD 自有) | UK(`part_code`,`sn`) |
