...
| D13 | MDM 同步策略 | Kafka 事件订阅为主 + Feign 全量快照兜底 + 本地 source 字段标注来源 | ① 仅 Feign 轮询 ② 仅 Kafka 事件 ③ 数据库 CDC | **解耦**：Kafka 事件异步消费，MDM 不可用时不阻塞 VMD 主流程。**可重放**：Kafka 事件支持 offset 回溯重放。**支持降级**：MDM 不可达时降级为只读，source=MANUAL 记录仍可本地维护。**幂等**：通过 external_ref_id + external_version 保证 upsert 幂等性。**适用实体（CR-011/CR-015/CR-016/CR-017/CR-018/CR-020 扩展）**：品牌 / 车系 / 平台 / **Plant（工厂）**/ **车型（Model）**/ **版本（Variant，原 BaseModel 基础车型）**/ **配置（Configuration，原 BuildConfig 生产配置）**/ **选项族（OptionFamily，原 FeatureFamily 特征族）/ 选项值（OptionCode，原 FeatureCode 特征值）**（以上来自 edd-mdm **Product MDM 子域**）/ **车载节点（VehicleNode，原 Device 设备，来自 edd-mdm EEAD 子域，CR-020）**统一采用本策略。详见「edd-mdm 接入规范」 |
...
| 表名 | PO 类 | 关键列 | 唯一约束 |
|------|------|--------|----------|
| `part` | `PartPo` | `pn`, `name`, `type`, `vehicle_node_code`（原 `device_code`，CR-020）, `supplier_code`, `software` | UK(`pn`) |
...
| `vehicle_part` | `VehiclePartPo` | `vin`, `pn`, `sn`, `vehicle_node_code`（原 `device_code`，CR-020）, `device_item`, `part_state`, `bind_org`, `bind_time`, `extra` | UK(`pn`,`sn`) |
...
**对应 US**：**US-001** / US-002 / US-003 / **US-004** / **US-005** / US-006 / **US-007** / **US-008** / **US-015**（MDM 事件同步 AC）。
...
sequenceDiagram
    ...
    participant FC as MdmBrandQueryClient / MdmCarLineQueryClient / MdmPlatformQueryClient / MdmPlantQueryClient / MdmModelQueryClient / MdmVariantQueryClient / MdmConfigurationQueryClient / MdmOptionFamilyQueryClient / MdmOptionCodeQueryClient / MdmVehicleNodeQueryClient
    ...
    alt 启动时自动
        ...
        VMD->>C: POST /api/mpt/mdmSync/v1/bootstrap?entity=brand|carLine|platform|plant|model|variant|configuration|optionFamily|optionCode|vehicleNode|all
...
**对应 US**：**US-001b** / US-002b / US-003b / **US-004b** / **US-005b** / US-006b / **US-007b** / **US-008b** / **US-015b**（Bootstrap 全量同步 AC）。
...
#### 5.1.17 MdmSync `MptMdmSyncController`（→ US-001b/US-002b/US-003b/US-004b/US-005b/US-006b/US-007b/US-008b/US-015b）
| Method | Path | Permission | Request | Response |
|--------|------|-----------|---------|----------|
| POST | `/api/mpt/mdmSync/v1/bootstrap` | `completeVehicle:mdmSync:bootstrap` | `?entity=brand\|carLine\|platform\|plant\|model\|variant\|configuration\|optionFamily\|optionCode\|vehicleNode\|all` | `ApiResponse<String>` |

**行为**：调用 MDM 全量快照接口拉取指定实体数据并 upsert 本地副本（不删除本地记录）。`entity=plant` 同步 Plant 投影（CR-011），`entity=model` 同步 Model 投影（CR-015），`entity=variant` 同步 Variant 投影（CR-016），`entity=configuration` 同步 Configuration 投影（CR-017），`entity=optionFamily` 同步 OptionFamily 投影、`entity=optionCode` 同步 OptionCode 投影（CR-018），`entity=vehicleNode` 同步 VehicleNode 投影（CR-020，来自 EEAD 子域），`entity=all` 含 Plant、Model、Variant、Configuration、OptionFamily / OptionCode 与 VehicleNode。
...
#### 5.2.1 MDM 快照查询 Feign 接口（→ US-001b/US-002b/US-003b/US-004b/US-005b/US-006b/US-007b/US-008b/US-015b）
